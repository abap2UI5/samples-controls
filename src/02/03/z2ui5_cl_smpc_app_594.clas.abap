...
" @origin sap.uxap.sample.ObjectPageSelectedSection - https://sdk.openui5.org/entity/sap.uxap.ObjectPageLayout/sample/sap.uxap.sample.ObjectPageSelectedSection (status: generated - machine-written, not yet reviewed)
...
CLASS z2ui5_cl_smpc_app_594 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    DO 6 TIMES.
      ...
      temp6 = sy-tabix.
      READ TABLE t_employees INDEX row_no ASSIGNING <temp5>.
      sy-tabix = temp6.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      blocks->ele( n = `Panel` ns = `m`
                  ...
                  )->a( n = `src` v = client->_bind( val = <temp1>-picture tab = t_employees tab_index = row_no )
              )->tag( n = `Label` ns = `m`
                  )->a( n = `text` v = client->_bind( val = <temp4>-name tab = t_employees tab_index = row_no )
              )->tag( n = `Label` ns = `m`
                  )->a( n = `text` v = client->_bind( val = <temp5>-job tab = t_employees tab_index = row_no )
