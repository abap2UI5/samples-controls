 ...
 *    No statement here carries it today; the rule is the half of this gate that
 *    has not fired yet, and it is here because the sibling sample
 *    repositories both shipped it.
 *
 ...
 *    `IS ASSIGNED` would then read TRUE for the failure. Twenty of these
 *    shipped here. One of them built every table row's supplier name from
 *    `COND #( WHEN sy-subrc = 0 … )` inside a LOOP, so a product could be
 *    given the PREVIOUS product's supplier.
 *
 ...
 *    with formal parameter V`, a SYNTAX_ERROR of the whole class. None here today; the rule travels with its two siblings, because any
 *    port that picks up a text symbol picks up the trap with it.
 *
