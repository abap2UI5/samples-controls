 ...
 *
 * The 2026-09-13 round added literal-no-separator, a KERNEL rule no offline
 * linter modelled at the time: a literal that ends where the next token begins
 * (`...`s_ctrl = ...), which reached a pulled main in two _event( ) wires -
 * apps 136 and 588, found by a user's Code Inspector run. It came out on
 * 2026-09-16: abaplint/abaplint#4289 closed the lexer gap and 2.120.52 reports
 * all three delimiter forms as a parser_error, which every repository here
 * already runs with parser_error on. The rule was the only thing that saw it
 * for three days; abaplint sees it now, and two checks of the same thing is
 * one to maintain too many.
 *
