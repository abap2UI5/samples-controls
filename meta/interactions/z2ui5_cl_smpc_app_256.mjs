// DateRangeSelectionHidden: three anchors (two Buttons, a Link) open ONE hideInput
// DateRangeSelection through a round-trip-free control_by_id openBy carrying
...
export default async (page, expect) => {
  await expect(page.locator('body'), 'the title').toContainText('Open Date Range Selection by Another Control');
  await expect(page.getByRole('button', { name: 'Open Date Range Selection', exact: true }).first(), 'the text anchor button').toBeVisibleEnabled();
  await expect(page.locator('a.sapMLnk').filter({ hasText: 'Open Date Range Selection' }).first(), 'the Link anchor').toBeVisible();
  await waitForUi5(page, () => {
    const dp = ui5All().find((c) => c.getMetadata().getName() === 'sap.m.DateRangeSelection' && c.getId().endsWith('HiddenDRS'));
  ...
  }, 'the hidden DateRangeSelection with its change wire and the three press-wired anchors did not all render');
};
